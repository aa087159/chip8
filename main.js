...
class VirtualMachine {
	...
	print_string_at_a() {
		...
		for (
			var i = this.reg_a;
			i < this.memory.length && this.memory[i] != 0;
			++i
		) {
...
var count_from_9_program = [
	0x20,
	0x30, // load addr 0x30 into A
	0x21,
	0x09, // load 0x09 into B

	// 0x04: top of loop

	// set up to print the nuber as ASCII
	0x31,
	0x30, // add offset 0x30 to B ('0' is 0x30, this gives us the ASCII
	// character rather than the raw number)
	0x10,
	0x00, // write B to addr at A
	0x61,
	0x30, // subtract the 0x30 off of B so we can use it as a counter
	0x40,
	0x00, // print string at A

	0x20,
	0x20, // load addr 0x20 into A (for " ,")
	0x40,
	0x00, // print string at A
	0x20,
	0x30, // load addr 0x30 into A (for numeric printing)

	0x61,
	0x01, // subtract 1 from B

	0x50,
	0x04, // Jump to address 0x04 if B is not 0
	// end of loop
...
// var vm = new VirtualMachine(hello_world_program);
// vm.run();
